...
binomial n 0 = 1
binomial 0 k = 0 -- We assume we wont be given k = 0, Otherwise we would need a Maybe Int in the signature
...
oddFactorial n = product (filter odd [1 .. n])
...
smallestDivisor n
  | n == 1 = 1
  | otherwise = divHelper n 2

divHelper :: Integer -> Integer -> Integer
divHelper n k
  | n `mod` k == 0 = k
  | otherwise = divHelper n (k + 1)
...
-- biggestPrimeAtMost n = fromJust $ find isPrime (reverse [2 .. n])
