...
maxBy :: (a -> Int) -> a -> a -> a
maxBy measure a b = if measure a < measure b then b else a
...
firstHalf s = case length s of
  1 -> s
  2 -> [head s]
  _ -> if even (length s) then take (length s `div` 2) s else take (length s `div` 2 + 1) s
...
capitalize s = unwords $ map capFirst $ words s

capFirst :: String -> String
capFirst s = toUpper (head s) : tail s
...
joinToLength l strs = [s | s <- [a ++ b | a <- strs, b <- strs], length s == l]
...
-- The fromRigth 0 e function is the same as either (const 0) id e
...
interpreterHelper (x, y) (c : cs) out = case c of
  "up" -> interpreterHelper (x, y + 1) cs out
  "down" -> interpreterHelper (x, y -1) cs out
  "right" -> interpreterHelper (x + 1, y) cs out
  "left" -> interpreterHelper (x -1, y) cs out
  "printY" -> interpreterHelper (x, y) cs (show y : out)
  "printX" -> interpreterHelper (x, y) cs (show x : out)
  _ -> interpreterHelper (x, y) [] out
interpreterHelper (_, _) [] out = reverse out
