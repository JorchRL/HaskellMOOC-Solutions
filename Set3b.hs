...
buildList start count end = h count [start, end]

h :: Int -> [Int] -> [Int]
h 0 (n : nums) = nums
h count (n : nums) = h (count -1) (n : n : nums)
...
mymaximum bigger initial (x:xs) = case bigger initial x of
  False -> mymaximum bigger x xs
  True -> mymaximum bigger initial xs
